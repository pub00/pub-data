...
#   This is the SAME pattern used in Git Helper and Secure Manager, adapted
#   for G-Manager (Chrome profile switcher).
#
# WHY >&2 EVERYWHERE?
...
# Usage: ui_header "Create New Profile"
#
...
# Usage: ui_info "Profile created successfully"
#        ui_error "Chrome is currently running"
ui_info()    { echo -e "  ${C_GREEN}[OK]${C_RESET} $1" >&2; }
ui_error()   { echo -e "  ${C_RED}[FAIL]${C_RESET} $1" >&2; }
ui_warn()    { echo -e "  ${C_YELLOW}[!!]${C_RESET} $1" >&2; }
ui_success() { echo -e "  ${C_GREEN}[DONE]${C_RESET} ${C_GREEN}$1${C_RESET}" >&2; }
...
# Usage:  NAME=$(ui_input "Enter email address")
#         DIR=$(ui_input "Profile name" "Account_test")
#
# FLAGS:
#   -r = raw (don't eat backslashes — important for emails with special chars)
...
ui_input() {
    ...
    # Build the display string: "> Enter email [default]: "
...
# --- ui_password ---
# Asks for sensitive input without showing characters.
# Usage:  TOKEN=$(ui_password "Enter secret")
#
# FLAG:
...
# Usage:  CHOICE=$(ui_menu "Pick Action" "Create" "Switch" "Quit")
#
# HOW IT WORKS:
...
# Usage:  DEST=$(ui_pick_dir "Select profile location")
#
# KEY DETAILS:
...
ui_pick_dir() {
    ...
    while true; do
        local display="  ${C_CYAN}[D]${C_RESET} ${prompt}"
        [[ -n "$default" ]] && display+=" ${C_DIM}[${default}]${C_RESET}"
        display+=": "
...
#
# Usage:
#   ui_spinner_start "Switching profile..."
#   some_long_command
#   ui_spinner_stop
#
# HOW IT WORKS:
...
ui_banner() {
    ...
    +-----------------------------------------------+
    |                                               |
    |          G-Manager -- Profile Switcher        |
    |                                               |
    |   Multi-Account Chrome . Create . Switch      |
    |                                               |
    +-----------------------------------------------+
EOF
    echo -e "${C_RESET}" >&2
