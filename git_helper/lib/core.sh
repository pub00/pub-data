...
core_clone_repo() {
    ...
    if [[ $pull_exit -ne 0 ]]; then
        ...
        pulled_branch="master"
        pull_output=$(git -C "$local_dir" pull origin master 2>&1)
        pull_exit=$?
    fi
    
    # If pull succeeded, rename the local branch to match the remote branch
    if [[ $pull_exit -eq 0 ]]; then
        git -C "$local_dir" branch -m "$pulled_branch" 2>/dev/null
    fi
...
core_push_no_delete() {
    ...
    if [[ $commit_exit -ne 0 ]]; then
        ...
        if echo "$output" | grep -q "nothing to commit"; then
            # Do not stop the spinner, proceed to push any existing local commits
            ui_info "No new files to commit. Proceeding to push..."
        else
            ui_spinner_stop
