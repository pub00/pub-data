...
echo -e "${GREEN}Selected: ${BOLD}${UCODE_PKG}${NC}"
...
if [[ "$INSTALL_MODE" == "2" ]]; then
    ...
    # Beautiful confirmation window showing passwords in clear text as requested
    clear
    echo -e "${BLUE}${BOLD}============================================================${NC}"
...
if [[ "$INSTALL_MODE" == "2" ]]; then
    # Fast Mode: Already collected partition details upfront
...
local mnt=$1     # The mount point (e.g., /mnt/snpsht.XXXXXX)
    local subvol=$2  # The subvolume name (e.g., @ or @_broken_...)
...
cleanup_broken() {
    ...
    # Find anything starting with @_broken_ or @_old_
...
preview_file "/etc/mkinitcpio.conf" "HOOKS" "Adding sd-encrypt, btrfs, and the secure Keyfile."
...
preview_file "/etc/default/grub" "GRUB CONFIGURATION" "Enabling Cryptodisk and setting the Kernel Keyfile for single-password login."

# 6a. Enable Cryptodisk
run_cmd "echo 'GRUB_ENABLE_CRYPTODISK=y' >> /etc/default/grub"

# 6b. Set Kernel CMDLINE (Simplified for automatic key detection)
